...
angular.module('schemaForm').config(
  ...
  .directive("toggleSingleModell", function() { //Rename from toggleSingleModel to toggleSingleModell to avoid conflict
...
angular.module('schemaForm').controller('dynamicSelectController', ['$scope', '$http', '$timeout', 'httpClient', '$translate',function ($scope, $http, $timeout, httpClient, $translate) {
   ...
   // $scope.form.options.scope = $scope;
    ...
    $scope.initializeUiSelect = function(value) {
			...
			separator = newOptions.map.separatorValue ? newOptions.map.separatorValue : ' - ';
				...
				if(typeof newOptions.map.nameProperty != 'string'){
					...
					for (var i in newOptions.map.nameProperty) {
    					newName += $translate.instant(current_row[newOptions .map.nameProperty[i]]); 
					...
					//if it is a string
					current_row["name"] = $translate.instant(current_row[newOptions .map.nameProperty]);
				}
    ...
    $scope.populateTitleMap = function (form, search) {
        ...
        if (form.schema && "enum" in form.schema) {
            ...
            form.schema.enum.forEach(function (item) {
                    form.titleMap.push({"value": item, "name": $translate.instant(item)})
                }
            );
        }
    ...
    $scope.find_in_titleMap = function (value) {
        for (i = 0; i < $scope.form.titleMap.length; i++) {
            if ($scope.form.titleMap[i].value == value) {
                if($scope.form.options.filter) { //MFE: Add this to cater for filters
                    if($scope.$parent.$eval($scope.form.options.filter, {item: $scope.form.titleMap[i]})){
                       return $scope.form.titleMap[i];
                    }
                } else {
                    return $scope.form.titleMap[i];//{"value": $scope.form.titleMap[i].value, "name": $scope.form.titleMap[i].name}
                }
        ...
        if ($scope.form.titleMap) {
            if (supplied_model !== undefined && angular.isArray(supplied_model)){
                supplied_model.forEach(function (value) {
                    	var found = $scope.find_in_titleMap(value)
                        if(found) {
                            $scope.internalModel.push(found);
                        }else{
                            var tmpIndex = $scope.externalModel.indexOf(value);
                            if(tmpIndex != -1)
                                $scope.externalModel.splice(tmpIndex, 1);
                        }
                    }
                )
            }
...
angular.module('schemaForm').filter('selectFilter', [function ($filter) {
    return function (inputArray, controller, localModel, strLocalModel) {
        ...
        //console.log("---> inputArray filtered", inputArray )
        ...
        var dataValues = pluck(data, ((controller.form.options.map) ? controller.form.options.map.valueProperty : "value"))
       ...
       // console.log("---> dataValues", dataValues )
        if (localModel) {
            ...
            if (controller.localModelType == "[object Array]") {
                ...
                var tmp = angular.copy(localModel);
