...
| ------------- |:-------------:|:-------------:|:-------------:|
  ...
  dateFormat | undefined | A function that accepts millisecond timestamps and formats them for display as chart labels. 
default. function (x) { return new Date(x).toString() }| NO
