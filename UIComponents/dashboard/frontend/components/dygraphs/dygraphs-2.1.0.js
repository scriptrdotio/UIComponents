...
var DEFAULT_ATTRS = {
  ...
  axes: {
    ...
    y: {
      axisLabelWidth: 75,
      pixelsPerLabel: 30,
      valueFormatter: function(data){return parseFloat(data);},
      axisLabelFormatter: function(data){return parseFloat(data);},//utils.numberAxisLabelFormatter,
      drawGrid: true,
      drawAxis: true,
      independentTicks: true,
  ...
  if ("development" != 'production') {
    ...
    OPTIONS_REFERENCE = // <JSON>
    {
      ...
      "offset": {
         ...
         "labels": ["Value display/formatting"],
         "type": "string",
         "description": "When set, display the graph dates with this offset."
      },
      "labelsUTC": {
        "default": "false",
...
var getDateAxis = function getDateAxis(start_time, end_time, granularity, opts, dg) {
  ...
  var date_array = [];
  date_array[DateField.DATEFIELD_Y] = accessors.getFullYear(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_M] = accessors.getMonth(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_D] = accessors.getDate(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_HH] = accessors.getHours(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_MM] = accessors.getMinutes(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_SS] = accessors.getSeconds(start_date,opts('offset'));
  date_array[DateField.DATEFIELD_MS] = accessors.getMilliseconds(start_date,opts('offset'));

  var start_date_offset = date_array[datefield] % step;
  if (granularity == Granularity.WEEKLY) {
    // This will put the ticks on Sundays.
    start_date_offset = accessors.getDay(start_date,opts('offset'));
  }
  ...
  if (granularity <= Granularity.HOURLY) {
    ...
    while (tick_time <= end_time) {
      if (granularity >= Granularity.DAILY || accessors.getHours(tick_date,opts('offset')) % step === 0) {
        ticks.push({ v: tick_time,
          label: formatter.call(dg, tick_date, granularity, opts, dg)
        });
      }
...
var getMomentOffset = function(d, interpret, offset) {
   if (interpret) {
     	//Passing true will keep the same local time, but at the expense of choosing a different point in Universal Time.
   		return moment(d).utcOffset(offset, true) 
   } else {
        //Passing false (the default) will keep the same instant in Universal Time, but the local time will change.
        return moment(d).utcOffset(offset, false) 
  }
}
    
/** Override dygraph to diplay dates according to the currently selected offset (which is stored in
 * offset) instead of UTC.
 * This function is only used if the 'labelsUTC' setting is true. See Dygraph
 * documentation regarding labelsUTC flag.
*/
var DateAccessorsUTC = {
      getFullYear:     function(d,offset) {
        return getMomentOffset(d, false,offset).year();
      },
      getMonth:        function(d,offset) {
        return getMomentOffset(d, false,offset).month();
      },
      getDate:         function(d,offset) {
        return getMomentOffset(d, false,offset).date();
      },
      getHours:        function(d,offset) {
        return getMomentOffset(d, false,offset).hour();
      },
      getMinutes:      function(d,offset) {
        return getMomentOffset(d, false,offset).minute();
      },
      getSeconds:      function(d,offset) {
        return getMomentOffset(d, false,offset).second();
      },
      getMilliseconds: function(d,offset) {
        return getMomentOffset(d, false,offset).millisecond();
      },
      getDay:          function(d,offset) {
        return getMomentOffset(d, false,offset).day();
      },
      makeDate:        function(y, m, d, hh, mm, ss, ms, offset) {
        return getMomentOffset(new Date(y, m, d, hh, mm, ss, ms), true, offset).toDate();
      }
};
...
function dateString_(time, utc, offset) {
  var accessors = utc ? DateAccessorsUTC : DateAccessorsLocal;
  var date = new Date(time);
  var y = accessors.getFullYear(date,offset);
  var m = accessors.getMonth(date,offset);
  var d = accessors.getDate(date,offset);
  var hh = accessors.getHours(date,offset);
  var mm = accessors.getMinutes(date,offset);
  var ss = accessors.getSeconds(date,offset);
  var ms = accessors.getMilliseconds(date,offset);
  // Get a year string:
  var year = "" + y;
...
function dateAxisLabelFormatter(date, granularity, opts) {
  ...
  var year = accessors.getFullYear(date,opts('offset')),
      month = accessors.getMonth(date,opts('offset')),
      day = accessors.getDate(date,opts('offset')),
      hours = accessors.getHours(date,opts('offset')),
      mins = accessors.getMinutes(date,opts('offset')),
      secs = accessors.getSeconds(date,opts('offset')),
      millis = accessors.getMilliseconds(date,opts('offset'));
...
function dateValueFormatter(d, opts) {
  return dateString_(d, opts('labelsUTC'), opts('offset'));
}
...
if(!this.resizeHandler_){this.resizeHandler_ = function(e){setTimeout(function(){dygraph.resize();},100)}; // Update when the window is resized.
...
var y0=isNullUndefinedOrNaN(axis.valueRange[0])?axis.extremeRange[0]:axis.valueRange[0];var y1=isNullUndefinedOrNaN(axis.valueRange[1])?axis.extremeRange[1]:axis.valueRange[1];axis.computedValueRange = [y0,y1];}else {axis.computedValueRange = axis.extremeRange;}if(!ypadCompat){ y0 = axis.computedValueRange[0];y1 = axis.computedValueRange[1];if (y0 === y1) {y0 -= 0.5;y1 += 0.5;} // When using yRangePad, adjust the upper/lower bounds to add
...
Legend.defaultFormatter = function (data) {
  ...
  for (var i = 0; i < data.series.length; i++) {
    ...
    html += "<span" + cls + "> <b><span style='color: " + series.color + ";'>" + series.labelHTML + "</span></b>:&#160;" + (( typeof series.yHTML != 'undefined' && series.yHTML != null) ? series.yHTML : "")  + "</span>";
  }
  return html;
};
...
rangeSelector.prototype.initInteraction_ = function () {
  ...
  var dragStartEvent =  'mousedown' //MFE: FIX FOR ZOOM SELECTOR AND GRIDSTER DRAG //window.opera ? 'mousedown' : 'dragstart';
